...
for j in range(0,100):
    for i in range(0,20):
        ...
        driver.get(url)
        try:
            address = wait.until(Ec.element_to_be_clickable((By.ID,'ContentPlaceHolder2_txtAddress'))).text
        except:
            address = ''
        ...
        try:
            ...
            #             f.write(data)
            urllib.request.urlretrieve(phone,'pics/' + str(j *20 + i) + 'phone.jpg')
        except:
            phone=''
...
data = data.transpose()
writer = pandas.ExcelWriter('jobs4.xlsx')
